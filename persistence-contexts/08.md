...
第 25 行的 `em.flush();` 可有可无，如果有必须在事务中，否则抛异常 javax.persistence.TransactionRequiredException: no transaction is in progress
...
第 38 行的 `session.flush();` 可有可无，如果有必须在事务中，否则抛异常 javax.persistence.TransactionRequiredException: no transaction is in progress
